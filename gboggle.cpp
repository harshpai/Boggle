...
//Extension: Embellish interface
//Function: DrawLineConnectingCubes
//------------------------------------------------------------
//Takes row and col for two cubes and draws a line connecting the 
//centre of two cubes
void DrawLineConnectingCubes(int row1,int col1, int row2,int col2){
	
	SaveGraphicsState();
	SetPenColor("ORANGE");
	double x1 = CubeX(col1) + gState.cubeSize/2;
	double y1 = CubeY(row1) + gState.cubeSize/2;
	double x2 = CubeX(col2) + gState.cubeSize/2;
	double y2 = CubeY(row2) + gState.cubeSize/2;
	double rad = 0.05;
...
{
	string s(1, ch);
	//Extension: Make Q a useful letter.
	if(ch=='Q')		s+="u";
  	SetFont(CUBE_FONT);
 	SetPointSize(gState.cubeFontSize);
